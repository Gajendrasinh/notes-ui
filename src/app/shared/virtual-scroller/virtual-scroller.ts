...
export class VirtualScrollerComponent implements OnInit, OnChanges, OnDestroy {
	...
	/** Calculate dynamic size item height function  */
	@Input() calculateItemHeightFn: (item) => number;
			...
			if (this.calculateItemHeightFn) {
				for (let i = 0; i < this.items.length; i++) {
					...
					if (!childSize) {
						...
						const childHeight = this.calculateItemHeightFn(this.items[i]);
						maxHeightForWrapGroup = Math.max(
							maxHeightForWrapGroup,
							childHeight
						);
